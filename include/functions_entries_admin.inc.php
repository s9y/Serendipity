...
function serendipity_printEntryForm($targetURL, $hiddens = array(), $entry = array(), $errMsg = "") {
    ...
    } elseif (!isset($entry['moderate_comments']) && (isset($serendipity['moderateCommentsDefault']) && serendipity_db_bool($serendipity['moderateCommentsDefault']))) {
    ...
    if (isset($entry['categories']) && is_array($entry['categories']) && !is_array($entry['categories'][0])) {
